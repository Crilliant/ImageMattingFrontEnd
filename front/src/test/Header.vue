...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            onBtn(img_or_upload_btn){
              if(img_or_upload_btn!="changeImg"){
                document.getElementById(img_or_upload_btn).click()
              }else{
                clearTimeout(this.clickTimer)
                const filename2=document.getElementById("changeImg");
                document.getElementById('changeImg').click()
                const pic_to_update = document.getElementById(img_or_upload_btn);
                filename2.onchange=function () {
                  pic_to_update.src = URL.createObjectURL(this.files[0]);
                }
              }
            },
            changeColor(){
